...
constexpr std::string_view kSettleSpeedKey = "settleSpeed";
constexpr std::string_view kGroundTiltKey = "groundTilt";
constexpr std::string_view kHeightOffsetKey = "heightOffset";
...
public:
  ...
  bool load() {
    ...
    if (!mConfig->save()) {
      mSelf.getLogger().warn("Loaded config but failed to persist normalization");
    }
    mRuntime.applyConfig(mConfig->value());
    mSelf.getLogger().info("Loaded Item Physics config from {}",
  ...
  bool enable() {
    ...
    const bool registered =
        pl::modmenu::ModuleBuilder(std::string(kModuleId), "Item Physics")
            .modId(mSelf.getId())
            .description("Java-style dropped item physics. Client-side renderer only.")
            ...
            .config(std::string(kRotationSpeedKey), "Tumble Speed",
                    pl::modmenu::ConfigType::SliderFloat,
                    numberText(snapshot.rotationSpeed),
                    numberText(kMinRotationSpeed), numberText(kMaxRotationSpeed))
            .config(std::string(kSettleSpeedKey), "Settle Speed",
                    pl::modmenu::ConfigType::SliderFloat,
                    numberText(snapshot.settleSpeed), numberText(kMinSettleSpeed),
                    numberText(kMaxSettleSpeed))
            .config(std::string(kGroundTiltKey), "Ground Tilt",
                    pl::modmenu::ConfigType::SliderFloat,
                    numberText(snapshot.groundTilt), numberText(kMinGroundTilt),
                    numberText(kMaxGroundTilt))
            .config(std::string(kHeightOffsetKey), "Height Offset",
                    pl::modmenu::ConfigType::SliderFloat,
                    numberText(snapshot.heightOffset), numberText(kMinHeightOffset),
                    numberText(kMaxHeightOffset))
    ...
    if (hookActive) {
      mSelf.getLogger().info("Item Physics enabled and registered in Mod Menu");
    } else {
  ...
  bool disable() {
    unregisterMenu();
    mRuntime.uninstall();
    mSelf.getLogger().info("Item Physics disabled");
    return true;
  }
...
private:
  ...
  void persistAndApplyLocked(std::string_view reason) {
    ...
    mRuntime.applyConfig(mConfig->value());
    if (mConfig->save()) {
      mSelf.getLogger().info("Persisted Item Physics config after {}", reason);
    } else {
      mSelf.getLogger().warn("Failed to persist Item Physics config after {}", reason);
    }
  }

  static void onToggle(std::string_view moduleId, bool enabled) {
    instance().handleToggle(moduleId, enabled);
  }
  ...
  void handleToggle(std::string_view moduleId, bool enabled) {
    if (moduleId != kModuleId) return;
    std::lock_guard lock(mConfigMutex);
    if (!mConfig) return;
    mConfig->value().enabled = enabled;
    persistAndApplyLocked("module toggle");
  }
  ...
  void handleConfigChanged(std::string_view moduleId, std::string_view key,
    ...
    if (key == kSingleModelKey) {
      config.singleModel = parseBool(value, config.singleModel);
    } else if (key == kRotationSpeedKey) {
      config.rotationSpeed = parseDouble(value, config.rotationSpeed);
    } else if (key == kSettleSpeedKey) {
      config.settleSpeed = parseDouble(value, config.settleSpeed);
    } else if (key == kGroundTiltKey) {
      config.groundTilt = parseDouble(value, config.groundTilt);
    } else if (key == kHeightOffsetKey) {
      config.heightOffset = parseDouble(value, config.heightOffset);
