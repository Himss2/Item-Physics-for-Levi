...
Version `0.9.2` keeps the device-approved airborne/full-block/decorated-pot,
water, contact, and fast render paths intact while completing the isolated
ground-height calibration for flat, shaped, normal-head, and Dragon Head
models.
...
- Ground offsets are tuned independently for flat, thin, shaped, head, special,
  ...
  use different render-origin corrections: normal heads use ground Y `+0.015`,
  while Dragon Head uses `-0.015`.
- The isolated ground calibration lowers flat 2D items to `-0.150` and shaped
  block items to `-0.165`. Full blocks, horizontal-thin items, special items,
  water lift, and all motion laws are unchanged.
