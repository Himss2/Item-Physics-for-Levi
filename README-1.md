...
Version `0.8.3` keeps the device-approved full-block path intact while fixing
head pivot/height, thin horizontal blocks, sneak stability, and frame spikes.
...
- Full 3D blocks keep the exact contact angle. Thin structural models such as
  ...
  rails use the flat contact pose only after landing.
...
- Per-entity traits are cached, state lookup probes at most eight slots, ground
...
then sneak/stand repeatedly while already-grounded items remain in view.
